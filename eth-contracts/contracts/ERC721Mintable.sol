...
contract Ownable {
    ...
    modifier onlyOwner(){
        require(msg.sender == _owner, "this function can only be called by the owner of the contract");
        _;
    }
    ...
    function transferOwnership(address newOwner) public onlyOwner {
        require(!newOwner.isContract(), "address is not a vilad");
        _owner = newOwner;
...
//    @dev Approves another address to transfer the given token ID
    function approve(address to, uint256 tokenId) public {
        ...
        require(msg.sender != to, "the new address is the same as the owner");

        // TODO require the msg sender to be the owner of the contract or isApprovedForAll() to be true
        require(_tokenOwner[tokenId] == msg.sender, "the sender is not the owner of the token");
    ...
    function _transferFrom(address from, address to, uint256 tokenId) internal {
        ...
        require(_tokenOwner[tokenId] == from, "the first address given is not the owner");

        // TODO: require token is being transfered to valid address
        require(!to.isContract(), "the address is not vilad");
...
contract ERC721Metadata is ERC721Enumerable, usingOraclize {
    ...
    function getter() external returns(string memory, string memory, string memory){
